...
import { getLabyrinth, imgFantomeCerise, setAllGhostBleu } from '../models/labyrinthModel.js';
...
heartImage.src = '../../img/coeur.png'; // Chemin vers l'image des cœurs
...
export function drawLabyrinth(ctx, labyrinth, numHearts) {
    for (let row = 0; row < labyrinth.length; row++) { // Parcourir les lignes du labyrinthe
        for (let col = 0; col < labyrinth[row].length; col++) { // Parcourir les colonnes de chaque ligne
            const x = col * cellSize; // Calculer la position x de la cellule
            const y = row * cellSize; // Calculer la position y de la cellule

            if (labyrinth[row][col] === 1) { // Si la cellule est un mur
...
export function drawPacGommes(ctx) {
    pacGommes.forEach((pacGomme) => {
        if (pacGomme.type === "bonus") { // Si c'est une Pac-Gomme bonus
            ...
            if (pointScore >= 500) { // Afficher la Pac-Gomme bonus si le score est suffisant
                ctx.drawImage(imgFantomeCerise, xCerise, yCerise, bonusWidth, bonusHeight);
            }
        } else {
...
function gameVictory() {
  ...
  // Arrête l'intervalle principal du jeu pour stopper les mises à jour.

  const canvas = document.querySelector("canvas");
  // Sélectionne le canvas du jeu (l'élément graphique principal).

  if (canvas) {
    canvas.remove();
    // Supprime complètement le canvas de la page, car le jeu est terminé.
  }
...
export function removeEatenPacGomme(pacmanPos) {
  ...
  pacGommes = pacGommes.filter((pacGomme) => {
    ...
    const isEaten = pacGomme.x === pacmanPos.col && pacGomme.y === pacmanPos.row;
    ...
    if (isEaten) {
      ...
      if (pacGomme.type === "big") {
        ...
        playSound('../audio/pacman_chomp.wav');
