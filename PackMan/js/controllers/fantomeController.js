...
import { getLabyrinth,getAllGhostBleu, labyrinth } from '../models/labyrinthModel.js';
...
import { pacman,resetPacmanPosition,live,lives } from '../models/pacmanModel.js';
import { checkCollisionWithGhosts } from "./pacmanController.js";
import { getGhostsPositions,resetGhostsPositions } from "../models/fantomesModel.js";
...
/*  Etape 1 : le pacman est la proie ******************************************* */
  ...
  function generateRandomPointAroundPacman() {
    ...
    if (!previousRandomPoint || (previousRandomPoint.row === possitionFantomeBleu.row && previousRandomPoint.col === possitionFantomeBleu.col)) {
  ...
  export function MoveBlueGhost(ctx) {
    ...
    if (nextMove) {
      ...
      if (possitionFantomeBleu.row === randomPoint.row && possitionFantomeBleu.col === randomPoint.col) {
  ...
  let previousRandomPointOrange = null;

// function generateRandomPointAroundOrange(pacman) {
//   const labyrinth = getLabyrinth(); // Récupérer le labyrinthe
//   const rows = labyrinth.length;
//   const cols = labyrinth[0].length;
...
//   // Définir une zone devant et à gauche de Pac-Man (rayon de 5 cases)
//   const minRow = Math.max(pacmanRow - 5, 0);
//   const maxRow = Math.min(pacmanRow - 1, rows - 1); // Devant Pac-Man
//   const minCol = Math.max(pacmanCol - 5, 0); // À gauche
//   const maxCol = Math.min(pacmanCol - 1, cols - 1); // Limite colonne gauche

//   // Générer un point uniquement si le précédent a été atteint ou n'existe pas
//   if (
//     !previousRandomPointOrange ||
//     (previousRandomPointOrange.row === possitionFantomeOrange.row &&
//       previousRandomPointOrange.col === possitionFantomeOrange.col)
//   ) {
//     let randomRow, randomCol;
//     do {
//       randomRow = Math.floor(Math.random() * (maxRow - minRow + 1)) + minRow;
//       randomCol = Math.floor(Math.random() * (maxCol - minCol + 1)) + minCol;
//     } while (!isValidMove(labyrinth, randomRow, randomCol)); // Vérifier que le point est valide

//     // Mettre à jour le point précédent
//     previousRandomPointOrange = { row: randomRow, col: randomCol };
//   }

//   return previousRandomPointOrange;
// }
...
function moveGhostAwayFromPacman(ghostPos, pacmanPos, labyrinth) {
  ...
  directions.forEach((direction) => {
    ...
    if (labyrinth[newRow] && labyrinth[newRow][newCol] === 0) {
      const distance = Math.abs(newRow - pacmanPos.row) + Math.abs(newCol - pacmanPos.col);
