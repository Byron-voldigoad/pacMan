...
import { pacman,resetPacmanPosition,lives } from '../models/pacmanModel.js';
...
import { drawLabyrinth, drawPacGommes,removeEatenPacGomme,victoriCondition } from '../vues/labyrinthVue.js';
...
function animateMovement(entity, startRow, startCol, endRow, endCol, duration, drawFunction) {
...
export function checkCollisionWithGhosts(pacman, ghostPosition) {
  return ghostPosition.some(ghost => ghost.row === pacman.row && ghost.col === pacman.col);
}
...
function movePacman(newRow, newCol, ctx) {
  ...
  animateMovement(
    ...
    () => {
      ...
      if (pacman.col === 49 && pacman.row === 8) {
        ...
        playSound('../audio/dragon_ball_z_tele.mp3');
      ...
      //     // Augmenter le score ou effectuer d'autres actions
      //     console.log("Score actuel:", eatenPacGommesCount);
      //     drawGhost(ctx)
      //   }
      // }else{
  ...
  //   // Appeler la fonction pour notifier la vue de redessiner
  //   redrawGame(ctx);
  //   if (live <= 0) {
